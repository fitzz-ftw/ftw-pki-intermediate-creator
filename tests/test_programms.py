...
from ftwpki.intermed.programms import prog_intermediate_csr, prog_intermediate_sign
...
def test_prog_intermediate_csr_success(tmp_path):
    ...
    # 2. Mocking
    with (
        patch("ftwpki.intermed.programms.getpass.getpass", return_value="geheim"),
        patch(
            "ftwpki.intermed.programms.toml2dn",
            return_value={
                "countryName": "DE",
        ...
        patch(
            "ftwpki.intermed.programms.PasswordManager.decrypt_password_file",
...
def test_prog_intermediate_csr_keyboard_interrupt(mocker):
    ...
    mocker.patch("ftwpki.intermed.programms.CSRIntermediateParser", side_effect=KeyboardInterrupt)
